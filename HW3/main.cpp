...
// Find index only
int SymbolTable::lookup_offset(string s){
    /* Found */
    auto iter = this->myMap.find(s);
    if (iter != this->myMap.end()){
        return iter->second + this->offset;
    }
    else{
        return nil;
    }
...
void SymbolTable::dump(){
    ...
    for (int i = 0; i < mapTemp.size(); i++){
        ...
        cout << " -index: " << this->lookup_offset(sym.myName) << '\n';
    }
    cout << '\n';
}
...
void SymbolTable_Scope::create(){
    ...
    if (this->myTables.size() == 1) table.setOffset(0); //Global don't count
    else table.setOffset(this->getOffset());
    this->myTables.push_back(table);
}

int SymbolTable_Scope::getOffset(){
    int offset = 0;
    for(int i = this->myTables.size() - 1; i >= 0; i--){
        offset += this->myTables[i].getTableSize();
    }
    return offset;
}
...
//Lookup the index
int SymbolTable_Scope::lookupSymbolIndex(string s){
    for(int i = this->myTables.size() - 1; i >= 0; i--){
        SymbolTable tmp = this->myTables[i];
        int idx = tmp.lookup(s);
        if (idx != nil){
...
//Lookup the index
int SymbolTable_Scope::lookupSymbolIndex_offset(string s){
    for(int i = this->myTables.size() - 1; i >= 0; i--){
        SymbolTable tmp = this->myTables[i];
        int idx = tmp.lookup_offset(s);
        if (idx != nil){
            return idx;
        }
    }
...
bool SymbolTable_Scope::isGlobal(string s){
    bool isLocal = false;
    bool isGlobal = false;
    for(int i = this->myTables.size() - 1; i >= 1; i--){
        SymbolTable tmp = this->myTables[i];
        int idx = tmp.lookup_offset(s);
        if (idx != nil){
            isLocal = true;
    ...
    if(this->myTables[0].lookup_offset(s) != nil){
        isGlobal = true;
    }    
